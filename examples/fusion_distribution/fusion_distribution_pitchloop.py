...
times = np.zeros((len(pitch_invs),sz)) # each pitch inverse row has corresponding loss frac at each time

i=0
for pitch_inv in pitch_invs:
    print('pitch_inv = ',pitch_inv)
    ...
    times[i,:], loss_frac_arr[i,:] = compute_loss_fraction(res_tys, tmin=tmin, tmax=tmax)
...
np.save("data/loss_frac_arr",loss_frac_arr)
np.save("data/pitch_invs",pitch_invs)
plt.figure()
